...
public class ExceptionUserRegistration {
    ...
    public static boolean emailId(String emailIds) {
        regex = "^[a-zA-Z-9]+([._+-]*[0-9A-Za-z]+)*@[a-zA-Z0-9]+.[a-zA-Z]{2,4}([.][a-z]{2,4})?$";
        Pattern pattern = Pattern.compile(regex);
        Matcher matcher = pattern.matcher(emailIds);
        return matcher.matches();
    }
    public static boolean testEmailId(String emailId) throws InputInvalidException {
        try {
            if (!emailId(emailId)) {
    ...
    public static boolean testMobileNumber(String mobileNumber) throws InputInvalidException {
        try {
